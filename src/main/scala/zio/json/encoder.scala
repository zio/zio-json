...
object syntax {
  implicit final class EncoderOps[A](private val a: A) extends AnyVal {
    def toJson(implicit A: Encoder[A]): String = A.toJson(a)
  }
}

// TODO pretty vs compact
trait Encoder[A] { self =>
  def toJson(a: A): String = {
    val writer = new internal.FastStringWriter(64)
    unsafeEncode(a, writer)
    writer.toString
  }
  ...
  final def contramap[B](f: B => A): Encoder[B] = new Encoder[B] {
    override def unsafeEncode(b: B, out: java.io.Writer): Unit = self.unsafeEncode(f(b), out)
...
object Encoder extends GeneratedTupleEncoders with EncoderLowPriority1 {
  ...
  implicit val string: Encoder[String] = new Encoder[String] {
    override def unsafeEncode(a: String, out: java.io.Writer): Unit = {
      out.write('"')
      var i   = 0
      val len = a.length
      while (i < len) {
  ...
  private[this] def explicit[A](f: A => String): Encoder[A] = new Encoder[A] {
    def unsafeEncode(a: A, out: java.io.Writer): Unit = out.write(f(a))
  }
  ...
  implicit def option[A](implicit A: Encoder[A]): Encoder[Option[A]] = new Encoder[Option[A]] {
    def unsafeEncode(oa: Option[A], out: java.io.Writer): Unit = oa match {
      case None    => out.write("null")
      case Some(a) => A.unsafeEncode(a, out)
    }
  ...
  implicit def either[A, B](implicit A: Encoder[A], B: Encoder[B]): Encoder[Either[A, B]] = new Encoder[Either[A, B]] {
    def unsafeEncode(eab: Either[A, B], out: java.io.Writer): Unit = {
      out.write("{")
      eab match {
        case Left(a) =>
          out.write("\"Left\":")
          A.unsafeEncode(a, out)
        case Right(b) =>
          out.write("\"Right\":")
          B.unsafeEncode(b, out)
      }
      out.write("}")
    }
  }
...
private[json] trait EncoderLowPriority1 {
  ...
  implicit def seq[A](implicit A: Encoder[A]): Encoder[Seq[A]] = new Encoder[Seq[A]] {
    def unsafeEncode(as: Seq[A], out: java.io.Writer): Unit = {
      out.write("[")
      var first = true
      as.foreach { a =>
  ...
  def keylist[K, A](
    ...
    def unsafeEncode(kvs: List[(K, A)], out: java.io.Writer): Unit = {
      out.write("{")
      var first = true
      kvs.foreach {
        case (k, a) =>
          if (!A.isNothing(a)) {
            ...
            string.unsafeEncode(K.unsafeEncodeField(k), out)
            out.write(":")
            A.unsafeEncode(a, out)
          }
      }
